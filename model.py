...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate, tau, batch_size):
        ...
        # Red Actuadora
        self.inputs, self.out, self.softmax = self.create_actor_network()

        self.network_params = tf.trainable_variables()

        # Red objetivo
        self.target_inputs, self.target_out, self.target_softmax = self.create_actor_network()
    ...
    def create_actor_network(self):
        inputs = tf.placeholder(tf.float32, [None, self.s_dim[0], self.s_dim[1]])

        net = tflearn.layers.conv.conv_1d(incoming=inputs,
                                          nb_filter=2,
                                          filter_size=3,
                                          strides=1,
                                          padding="valid",
        ...
        out = tf.cast(tf.argmax(softmax, axis=1), tf.int32)

        return inputs, out, softmax

    def train(self, inputs, a_gradient):
        self.sess.run(self.optimize, feed_dict={self.target_inputs: inputs,
    ...
    def predict_target_softmax(self, inputs):
...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        ...
        net = tflearn.fully_connected(net, 400, weights_init=self.initializer,)
    ...
    def train(self, inputs, action, predicted_q_value):
        return self.sess.run([self.out, self.loss, self.optimize], feed_dict={self.inputs: inputs,
                                                                   self.action: action,

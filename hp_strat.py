...
def initialize(context):
    ...
    context.training_data_size = 950000  # 950000
...
def handle_data(context, data):
    if True: #context.i % context.bar_period == 0:
        ...
        if signal != 0:
            ...
            if signal == 1 and confirmation == 1:
                ...
                for i, asset in enumerate(context.symbols):
                    ...
                    print("Compra: ", size)
            elif signal == -1 and confirmation == 1:
                print(context.meta_model.predict_proba(confirmation_input).shape)
                print(context.meta_model.predict_proba(confirmation_input))
                size = context.meta_model.predict_proba(confirmation_input)[0][1]
                for i, asset in enumerate(context.symbols):
                    order_target_percent(asset, -size)
                    print("Venta: ", size)
...
def train_model(context, _data):
    ...
    print("Inicio de conjunto de entrenamiento: ", data.index[0])
    print("Fin de conjunto de entrenamiento: ", data.index[-1])
    ...
    ##########
    Está basado en la idea de crear eventos de trading enmarcados con un tiempo de inicio, 
    un tiempo final y un horizonte máximo.
                
    ##########
    # Estrategia sub-yacente basada en "High Probability Trading Strategies" de Robert C. Miner
    ##########
    Para crear la lista de eventos vamos a utilizar una estrategia basada en algunos de los tópicos
    tratados en libro de Robert C. Miner. El objetivo de la estrategia en el algoritmo es definir
    los eventos que serán pasados al modelo como datos de entrenamiento. De esta manera el modelo
    aprende la estrategia sub-yacente con la que fueron generados los datos.
    
    En esta etapa el modelo solo sugiere el lado de la operación, pero no la cantidad a invertir.
    
    ##########
    Empecemos por definir el primer elemento lógico de la estrategia: Momentum con un doble marco temporal.
    El momentum en el marco temporal superior servirá como filtro y la reversión del momentum en el marco
    temporal inferior servirá como disparador del evento. 
    
    Se debe buscar el momentum mayor sea un periodo 4 veces mayor al momentum menor.
    """
    ...
    # Ahora debemos establecer los índices (Datetime) de los eventos
    dmsIndex = features[features["DMStrat"] != 0]["DMStrat"].index
    tEvents = pd.DatetimeIndex(dmsIndex)
    ...
    print("SML INFO: Inicia el entrenamiento")
